...
# after importing the u2rNew library you can get some information using these commands:
# help(u2rNew)
print(u2rNew.simple_interpolate_from_mesh_to_grid.__doc__)
...
value_mesh = np.zeros((ndim_field,nNodes,1))
x_all = np.zeros((2,nNodes))
# ddx = np.zeros((3))
# block_x_start = np.zeros((1))
# nx = np.zeros((1))
# nx = np.zeros((1))
# nx = np.zeros((1))
...
zeros_on_mesh = 0
value_grid = u2rNew.simple_interpolate_from_mesh_to_grid(value_mesh,x_all,x_ndgln,ddx,block_x_start,nx,ny,nz,zeros_on_mesh,nEl,nloc) #,nNodes,nscalar,ndim,nTime)
# value_grid = u2rNew.simple_interpolate_from_mesh_to_grid(value_mesh, x_all,x_ndgln,ddx,block_x_start,nx,ny,nz, nNodes,nEl,nloc,nscalar,ndim,nTime)
# value_mesh, x_all,x_ndgln,ddx,block_x_start,nx,ny,nz, nNodes,nEl,nloc,nscalar,ndim,nTime
...
zeros_on_grid = 1
value_remesh = u2rNew.interpolate_from_grid_to_mesh(value_grid, block_x_start, ddx, x_all, zeros_on_grid, nscalar, nx, ny, nz, nNodes, ndim, nTime)
