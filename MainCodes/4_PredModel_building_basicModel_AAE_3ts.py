...
# Initialize lists to store losses and epochs for plotting
all_losses_autoencoder = []
all_losses_discriminator = []
epochs = []
...
for epoch in range(num_epochs):
    ...
    all_losses_autoencoder.append(loss_autoencoder.item())
    all_losses_discriminator.append(loss_adversarial.item())
    epochs.append(epoch + 1)

    # Print the losses at the end of each epoch
    print(f'Epoch [{epoch+1}/{num_epochs}], Autoencoder Loss: {loss_autoencoder.item():.4f}, Discriminator Loss: {loss_adversarial.item():.4f}')

print("Training finished.")

model_saved_path = root_path + 'Flow_Data/PredictionModel/autoencoder_Flow_PredictionLatent_{}epochs_AAE80Samples_3ts.pth'.format(num_epochs)
...
plt.subplot(1, 2, 1)
plt.plot(epochs, all_losses_autoencoder, label='Autoencoder Loss')
...
plt.subplot(1, 2, 2)
plt.plot(epochs, all_losses_discriminator, label='Discriminator Loss')
...
plt.tight_layout()
fig.savefig(model_saved_path[:-5]+'1.png')
plt.close(fig)
...
plt.plot(epochs, all_losses_autoencoder, label='Autoencoder Loss')
plt.plot(epochs, all_losses_discriminator, label='Discriminator Loss')
plt.title('Autoencoder and Discriminator Losses')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()

plt.tight_layout()
fig.savefig(model_saved_path[:-5]+'2.png')
plt.close(fig)
